<?php
/**
 * Irfeedback main file
 *
 * @package ImageRecycle\ImageRecycle\Irfeedback
 */

namespace ImageRecycle\ImageRecycle\Irfeedback;
...
class Irfeedback
{
    ...
    public static function init($main_plugin_file, $plugin_prefix, $plugin_slug, $plugin_name, $text_domain)
    {
        ...
        add_action('admin_init', array(__CLASS__, 'irfeedbackCallAjax'));

        // ADD REVIEW NOTICE FOR PLUGIN
        //
        //
        register_activation_hook(self::$main_plugin_file, array(__CLASS__, 'irreviewActivation'));
        register_deactivation_hook(self::$main_plugin_file, array(__CLASS__, 'irreviewDeactivation'));

        $install_time = get_option(self::$plugin_prefix . '_irreview_installation_time');
        if (!empty($install_time) && $install_time !== 'unnecessary' && ($install_time + (30 * 24 * 60 * 60) < time())) {
            add_action('admin_notices', array(__CLASS__, 'irreviewNotice'));
        }
    }
     ...
     * @return void
     */
    public static function irfeedbackCallAjax()
    {
        if (current_user_can('manage_options')) {
            add_action('wp_ajax_ir_send_feedback_deactive_' . self::$plugin_slug, array(__CLASS__, 'sendFeedbackDeactive'));
            add_action('wp_ajax_ir_disable_feedback_' . self::$plugin_slug, array(__CLASS__, 'disableFeedback'));
            add_action('wp_ajax_irreview_ajax_hide_review_' . self::$plugin_slug, array(__CLASS__, 'ajaxHideReview'));
            add_action('wp_ajax_ir_feedback_get_technical_data_' . self::$plugin_slug, array(__CLASS__, 'ajaxGetTechnicalData'));
        }
     ...
     * @return void
     */
    public static function irUpdatePlugin()
    {
        $ir_version_installed = get_option(self::$plugin_prefix . '_irfeedback_version', false);

        if (!$ir_version_installed) {
            update_option(self::$plugin_prefix . '_irreview_installation_time', time());
            // Update current version
            update_option(self::$plugin_prefix . '_irfeedback_version', '1.0.0');
    ...
    public static function enqueueFeedbackScript()
    {
        ...
        wp_enqueue_style(
            'ir-feedback-style',
            plugin_dir_url(self::$main_plugin_file) . 'irfeedback/assets/css/irfeedback.css'
        );
        ...
        wp_enqueue_script(
            'ir-feedback-tooltip',
            plugin_dir_url(self::$main_plugin_file) . 'irfeedback/assets/js/tooltip.js'
        );
        wp_enqueue_script(
            'ir-feedback-global',
            plugin_dir_url(self::$main_plugin_file) . 'irfeedback/assets/js/global.js'
        );
        wp_enqueue_script(
            'ir-feedback-velocity',
            plugin_dir_url(self::$main_plugin_file) . 'irfeedback/assets/js/velocity.min.js'
        );
        wp_enqueue_script(
            'ir-feedback',
            plugin_dir_url(self::$main_plugin_file) . 'irfeedback/assets/js/irfeedback.min.js'
        );

        wp_localize_script('ir-feedback', 'ir_feedback', array(
            'token' => wp_create_nonce('ir-feedback'),
    ...
    public static function renderFeedbackModal()
    {
        ...
        <div id="<?php echo esc_attr(self::$plugin_prefix) ?>_feedback_modal"
             class="ir-feedback-dialog <?php echo esc_attr(self::$plugin_slug) ?>" style="display: none">
            ...
            <div class="feedback-button">
                ...
                <img class="ir-loading"
                     src="<?php echo esc_html(plugin_dir_url(self::$main_plugin_file) . 'irfeedback/assets/loading.gif') ?>"/>
                <div class="clear"></div>
            </div>
        </div>
        <?php
    ...
    {
        check_ajax_referer('ir-feedback', 'ajax_nonce');
    ...
    public static function ajaxGetTechnicalData()
    {
        ...
        // Load IrCheckDebugData class
        if (!class_exists('IrCheckDebugData')) {
            require_once(plugin_dir_path(self::$main_plugin_file) . 'irfeedback/ir-check-debug-data.php');
        }

        // Get server information
        $technical_data = call_user_func('IrCheckDebugData::debugData');
        // Filter data
    ...
    {
        check_ajax_referer('ir-review', 'ajaxnonce');
        update_option(self::$plugin_prefix . '_irreview_installation_time', 'unnecessary');
        wp_die();
    }
     ...
     * @return void
     */
    public static function irreviewActivation()
    {
        $install_time = get_option(self::$plugin_prefix . '_irreview_installation_time');
     ...
     * @return void
     */
    public static function irreviewDeactivation()
    {
        $install_time = get_option(self::$plugin_prefix . '_irreview_installation_time');
        if (!empty($install_time) && $install_time !== 'unnecessary' && ($install_time + (7 * 24 * 60 * 60) > time())) {
            delete_option(self::$plugin_prefix . '_irreview_installation_time');
        }
    }
     ...
     * @return void
     */
    public static function irreviewNotice()
    {
        wp_enqueue_script(
            'ir-review',
            plugin_dir_url(self::$main_plugin_file) . 'irfeedback/assets/js/irreview.min.js'
        );
        wp_localize_script('ir-review', 'ir_review', array(
            'token' => wp_create_nonce('ir-review'),
            'ajaxurl' => admin_url('admin-ajax.php')
        ));
        //phpcs:disable WordPress.WP.I18n.NonSingularStringLiteralDomain -- Already use text domain for extensions
        echo '<div id="' . esc_attr(self::$plugin_prefix) . '-review-notice" class="updated irreview-notice" data-slug="' . esc_attr(self::$plugin_slug) . '">
	          <p>' . sprintf(
            ...
            esc_html(self::$plugin_name)
	          ...
	          <p class="submit">
	             <a href="' . esc_html('https://wordpress.org/support/plugin/' . self::$plugin_slug . '/reviews/?filter=5#new-post') . '" target="_blank" class="button-primary irreview-already-review">' . esc_html__('Sure I’d love to', self::$text_domain) . '</a>
	             <button class="button-secondary irreview-hide-review">' . esc_html__('No hide notification', self::$text_domain) . '</button>
	          </p>
	          </div>';
        //phpcs:enable
    }
